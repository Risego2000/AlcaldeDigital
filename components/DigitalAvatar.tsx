...
const DigitalAvatar: React.FC<DigitalAvatarProps> = ({ status, volume = 0 }) => {
  ...
  // Decidimos usar localUrl por defecto para evitar el error ERR_NAME_NOT_RESOLVED del enlace caducado de Supabase
  const avatarUrl = localUrl;
  ...
  return (
    <div className="relative w-[300px] h-[300px] sm:w-[400px] sm:h-[400px] md:w-[500px] md:h-[500px] flex items-center justify-center avatar-jarvis-vars">
      ...
      <div
        className={`absolute rounded-full transition-all duration-700 ease-out blur-[80px] opacity-40 ${isSpeaking ? 'bg-blue-500' : isListening ? 'bg-amber-400' : isConnecting ? 'bg-white' : 'bg-blue-900/10'
          }`}
        style={{
          width: '70%',
          height: '70%',
      ...
      <div className={`relative w-[200px] h-[200px] sm:w-[280px] sm:h-[280px] md:w-[350px] md:h-[350px] rounded-full z-20 transition-all duration-700 overflow-hidden ${isSpeaking ? 'scale-[1.02]' : 'scale-100'
        }`}>

        {/* The Frame Border (Jarvis Style Glow) */}
        <div className={`absolute inset-0 rounded-full border-2 transition-colors duration-500 z-50 ${isSpeaking ? 'border-blue-400' : isListening ? 'border-amber-400' : 'border-white/20'
          }`} style={{
        ...
        <div className={`w-full h-full relative transition-all duration-1000 ${isIdle ? 'animate-avatar-breath grayscale-0 opacity-100' : 'grayscale-0 opacity-100'}`}>
          <canvas
            ref={canvasRef}
            className={`absolute inset-0 w-full h-full object-cover z-30 transition-opacity duration-700 ${(!isSpeaking || smoothedVolume < 0.01) && imageLoaded ? 'opacity-100' : 'opacity-0 pointer-events-none'
      ...
      <div className="absolute inset-0 pointer-events-none z-10">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className={`absolute w-1 h-1 bg-blue-400 rounded-full transition-all duration-1000 ${isSpeaking ? 'opacity-80' : 'opacity-0'
              }`}
            style={{
              top: '50%',
              left: '50%',
              ...
              animationName: isSpeaking ? 'jarvisParticle' : 'none',
